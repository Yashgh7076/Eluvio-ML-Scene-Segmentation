...
conv1.add(tf.keras.layers.Conv1D(filters = 6, kernel_size = 15, strides = 1, padding='same', activation='linear')) #input size (None, 2048, 4)
conv1.add(tf.keras.layers.BatchNormalization(axis=2))
conv1.add(tf.keras.layers.ReLU())
conv1.add(tf.keras.layers.MaxPool1D(pool_size = 2)) #output size (None, 1024, 6)

conv1.add(tf.keras.layers.Conv1D(filters = 6, kernel_size = 15, strides = 1, padding='same', activation='linear')) #input size (None, 1024, 6)
conv1.add(tf.keras.layers.BatchNormalization(axis=2))
conv1.add(tf.keras.layers.ReLU())
conv1.add(tf.keras.layers.MaxPool1D(pool_size = 2)) #output size (None, 512, 6)

conv1.add(tf.keras.layers.Conv1D(filters = 8, kernel_size = 7, strides = 1, padding='same', activation='linear')) #input size (None, 512, 6)
conv1.add(tf.keras.layers.BatchNormalization(axis=2))
conv1.add(tf.keras.layers.ReLU())
conv1.add(tf.keras.layers.MaxPool1D(pool_size = 2)) #output size (None, 256, 8)

conv1.add(tf.keras.layers.Conv1D(filters = 8, kernel_size = 7, strides=1, padding='same', activation='linear')) #input size (None, 256, 8)
conv1.add(tf.keras.layers.BatchNormalization(axis=2))
conv1.add(tf.keras.layers.ReLU())
conv1.add(tf.keras.layers.MaxPool1D(pool_size = 2)) #output size (None, 128, 8)

conv1.add(tf.keras.layers.Conv1D(filters = 8, kernel_size = 5, strides=1, padding='same', activation='linear')) #input size (None, 128, 8)
conv1.add(tf.keras.layers.BatchNormalization(axis=2))
conv1.add(tf.keras.layers.ReLU())
conv1.add(tf.keras.layers.MaxPool1D(pool_size=2)) #output size (None, 64, 8)

conv1.add(tf.keras.layers.Conv1D(filters=8, kernel_size = 5, strides=1, padding='same', activation='linear')) #input size (None, 64, 8)
conv1.add(tf.keras.layers.BatchNormalization(axis=2))
conv1.add(tf.keras.layers.ReLU())
conv1.add(tf.keras.layers.MaxPool1D(pool_size = 2)) #output size (None, 32, 8)


encoded_left = conv1(shot_1)
encoded_right = conv1(shot_2)

combine = tf.keras.layers.concatenate([encoded_left, encoded_right], axis = 2) #output size (None, 32, 16)
flat = tf.keras.layers.Flatten()(combine) #output size (None, 512)
...
d3 = tf.keras.layers.Dense(10, activation='relu')(d2d)
d3r = tf.keras.layers.Dropout(rate=0.5)(d3)
d3d = tf.keras.layers.BatchNormalization(axis=1)(d3r)

output = tf.keras.layers.Dense(1, activation = 'softmax')(d3d)
